...
void World::generateWorld() {
  initWorld();
  if (world.empty()) {
    std::cout << "Initialization of world failed! Your world is empty!"
              << std::endl;
  }
  ...
  for (curPos.first = 0; curPos.first < size; curPos.first++) {
    for (curPos.second = 0; curPos.second < size; curPos.second++) {
      ...
      spriteList.at(curPos.first).at(curPos.second) =
          currentGround.getSpriteName();

      int treeChance = rand() % 100;

      RandTreeGenerator treeGen(forestPath,
                                std::make_shared<WorldState>(worldState));
      TreeTrunk *newTrunk = treeGen.generateTreeTrunk();
      if (treeChance < 5 && !hasTreeInArea(newTrunk->getDistanceTolerance()) &&
          !currentGround.getTreeTrunk()) {
...
void World::populateWorld() {
  ...
  while (!curPosAtEnd()) {
    ...
    RandShroomGenerator shroomGen(forestPath,
                                  std::make_shared<WorldState>(worldState));
    Mushroom *newShroom = shroomGen.generateMushroom();
    if (mushChance < 7 && !currentGround.hasTreeTrunk() &&
        !currentGround.hasMushroom() &&
...
Ground *World::getNearCurPos(int hor, int vert) {
  if (0 <= curPos.first + hor && curPos.first + hor < size &&
      0 <= curPos.second + vert && curPos.second + vert < size) {
...
std::vector<std::vector<std::string>> World::getSpriteNames() {
  return spriteList;
}
